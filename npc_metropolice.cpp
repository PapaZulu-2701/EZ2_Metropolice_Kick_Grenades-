...
int CNPC_MetroPolice::SelectRangeAttackSchedule()
{
	...
	if( TryToEnterPistolSlot( SQUAD_SLOT_ATTACK1 ) || TryToEnterPistolSlot( SQUAD_SLOT_ATTACK6 ))
...
float CNPC_MetroPolice::StitchAtWeight( float flDist, float flSpeed, float flDot, float flReactionTime, const Vector &vecTargetToGun )
{
	...
	if( IsStrategySlotRangeOccupied( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK6 ) )
		return false;
...
int CNPC_MetroPolice::SelectStitchSchedule()
{
	...
	if( OccupyStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK6 ) )
	{
		if ( IsInSquad() && (i < 2) )
		{
...
bool CNPC_MetroPolice::OnBeginMoveAndShoot()
{
	...
	{
		if( HasStrategySlotRange( SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK6 ) )
...
void CNPC_MetroPolice::StartTask(const Task_t* pTask)
{
	...
	{
		DevMsg("CP StartTask: Iniciando TASK_METROPOLICE_KICK_LAUNCH.\n");	// Logs a debug message indicating the task has started.
		...
		{
			DevMsg("CP StartTask: ERRO em KICK_LAUNCH! Tentou chutar, mas m_hPulledObject estava nulo.\n"); // Logs an error message if no grenade was memorized.
		}
	...
	case TASK_METROPOLICE_PLAY_KICK_ANIMATION: // [MODIFICATION]
	{
		...
		if (kickSequence != ACTIVITY_NOT_AVAILABLE)
		{
			...
			// [NEW LINE] Increases the animation playback speed by 1.5x.
			m_flPlaybackRate = 1.50f; //This seems the best speed to look more "natural"
...
//=========================================================
// [MODIFICATION]
// Schedule for the kick grenade ability
//=========================================================
